...
class HeaderPainter extends CustomPainter {
  ...
  _getSkyColors() {
    ...
    var topSkyColor = Color.lerp(Colors.indigo.shade700, Colors.lightBlueAccent.shade700, lerpValue);
    var bottomSkyColor = Color.lerp(Colors.indigo.shade100, Colors.lightBlueAccent.shade100, lerpValue).withOpacity(0.8);
  ...
  _drawSky(Canvas canvas, Size size){
    ...
    canvas.drawRect(
      ...
      Paint()
        ..shader = skyGradient.createShader(Rect.fromLTWH(0.0, 0.0, size.width, size.height)),
    );
  }

  _drawArc(Canvas canvas, Size size, double pathLineToHeight, double endPointHeight, double controlPointHeight, double opacity){
    var path = Path()
      ..moveTo(0.0, size.height)
      ..lineTo(size.width, size.height)
      ..lineTo(size.width, size.height - pathLineToHeight);
  ...
  _drawStar(Canvas canvas, double dx, double dy, double radius, double opacity){
    ...
    canvas.drawCircle(startCenter, starGradientRadius, Paint()..shader = circleGradientShader);
    canvas.drawCircle(startCenter, radius, Paint()..color = Colors.white.withOpacity(opacity));

  }

  _drawFallingStar(Canvas canvas, double dx, double dy, double h, double r, num alpha, double opacity) {
    ...
    canvas.drawPath(path, Paint()..shader = fallingStarGradient.createShader(Rect.fromLTRB(dx, dy, cx, cy),),);
  }

  _drawSun(Canvas canvas) {
    var sunCenter = Offset(60.0, 90.0);
    var sunInnerGradientRadius = 35.0;
    var circleInnerGradientShader = RadialGradient(
      colors: [
        Colors.white.withOpacity(0.5),
        Colors.white.withOpacity(0.0),
      ]
    ).createShader(Rect.fromCircle(center: sunCenter, radius: sunInnerGradientRadius));
    canvas.drawCircle(sunCenter, sunInnerGradientRadius, Paint()..shader = circleInnerGradientShader);
